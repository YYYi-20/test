...
def normalize_staining(img,
    ...
    HERef = np.array([[0.5077, 0.1859], [0.7618, 0.8696], [0.4024, 0.4574]])
    ...
    ODhat = OD[~np.any(OD < beta, axis=1)]  # 每一行的三列同时大于beta

    # 即 小于 pixel  np.exp(-beta)*255-1
    ...
    if savePath is not None:
        ...
        Image.fromarray(Inorm).save(savePath, quality=80)
    else:
        return Inorm
