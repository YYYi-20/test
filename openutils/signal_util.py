...
def exponential_running_standardize(data,
    ...
    if init_block_size is not None:
        ...
        init_mean = np.mean(data[0:init_block_size],
                            axis=other_axis,
                            keepdims=True)
        init_std = np.std(data[0:init_block_size],
                          axis=other_axis,
                          keepdims=True)
...
class slide_window():
    def __init__(self, data, size, step):
        self.data = data
        self.size = size
        self.step = step
    ...
    def apply(self, fun, start=0, end=-1):
        data = self.data.copy()
        shape = data.shape
        if end == -1:
            end = 701
